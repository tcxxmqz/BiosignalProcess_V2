...
def _eda_findpeaks_nabian2018qz(eda_phasic):
    ...
    ----------
     ...
     同时在返回值中添加SCR_Offsets; SCR幅值<0.1*最大SCR幅值（2020-10-29已修改正确）

     modified by qizhen in 2020-10-29 14：58
    ...
    # modified by qz
    info = {"SCR_Onsets": np.array(onsets_list),
            "SCR_Offsets": np.array(offsets_list),
            "SCR_Peaks": np.hstack(np.array(peaks_list)),
            "SCR_Height": np.array(amps_list),
            "eda_phasic_diffandsmoothed": eda_phasic_diffandsmoothed}
