...
def emg_clean(emg_signal, sampling_rate=1000):
    ...
    # b, a = scipy.signal.butter(N=order, Wn=frequency, btype="lowpass", analog=False)
...
def emg_clean_qz(emg_signal, sampling_rate=2000):
    """Preprocess an electromyography (emg) signal.
...
def emg_clean_qz_ellip(emg_signal, sampling_rate=2000):
    """Preprocess an electromyography (emg) signal.
    ...
    # Filtering
    b, a = scipy.signal.ellip(N=order, Wn=frequency, rp=3, rs=50, btype='bandpass')
    filtered = scipy.signal.filtfilt(b, a, emg_signal)
