...
def ecg(signal=None, sampling_rate=1000., show=True, plot_method=None, path=None):
    ...
    if plot_method in ["orign", "biosppy"]:
        plotting.plot_ecg(ts=ts,
                          ...
                          path=path,
                          show=show)
    elif plot_method in ["qz", "qizhen"]:
        plotting.plot_ecg_qz(heart_rate_ts=ts_hr,
                             heart_rate=hr,
                             path=path,
                             show=show)
    else:
        raise ValueError("biosppy error: ecg(): 'plot_method' should be one of 'biosppy'" "or 'qz'.")
