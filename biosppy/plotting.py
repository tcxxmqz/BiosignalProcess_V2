...
def plot_emg_qz(ts=None, emg_cleaned=None, path=None, show=None):
    """
    绘制肌电信号图像。

    :param ts: 时间序列
    :param emg_cleaned: 滤波后的肌电信号
    :param path: 保存的文件路径
    :param show: 是否显示
    :return: 无
    """
    ...
    if path is not None:
        ...
        fig.savefig(path, dpi=300, bbox_inches='tight')
        print("plot_emg_qz()处理后的图像已经保存到文件路径{}".format(path))
...
def plot_ecg_qz(heart_rate_ts=None,
             heart_rate=None,
             path=None,
             show=False):
    """每分钟心率BPM曲线绘图
    ...
    if path is not None:
        ...
        fig.savefig(path, dpi=300, bbox_inches='tight')
        print("plot_ecg_qz()处理后的图像已经保存到文件路径{}".format(path))
